...
where releaseTimes[i] was the time the ith key was released. Both arrays are 0-indexed. The 0th key was pressed at the time 0, and every subsequent key was
...
class Solution:
    def slowestKey(self, releaseTimes, keysPressed: str) -> str:
        ...
        releaseTimes = [(releaseTimes[c]-releaseTimes[c-1], keysPressed[c]) if c != 0 else (releaseTimes[c], keysPressed[c]) for c in range(0, len(releaseTimes))]
        return sorted(releaseTimes, key=lambda e: (e[0], e[1]))[-1][1]